    ...
    <div className="mx-auto w-full max-w-3xl px-3 pb-3 sm:px-4 sm:pb-4">
      ...
      >
        <textarea
          ...
          onKeyDown={onKeyDown}
          rows={1}
          placeholder="Message SPYRO V1…  (Enter to send, Shift+Enter for new line)"
          className={cn(
          ...
          <AnimatePresence mode="wait" initial={false}>
            {isGenerating ? (
              <motion.button
                ...
                onClick={onStop}
                className="grid h-9 w-9 place-items-center rounded-xl bg-muted text-foreground transition-colors hover:bg-muted/70"
              ...
              <motion.button
                ...
                disabled={!value.trim()}
                className={cn(
                  "grid h-9 w-9 place-items-center rounded-xl spyro-bg-gradient text-primary-foreground transition-all",
