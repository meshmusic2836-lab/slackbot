...
function stripAttribution(text: string): string {
  ...
  // 1. Kill the big "Support Pollinations.AI" ad block (most common).
  //    Pattern: optional --- separator, "Support Pollinations.AI" heading,
  //    --- separator, then the 🌸 Ad 🌸 block with kofi link.
  out = out.replace(
    /\n*---\s*\n+\*\*Support Pollinations\.AI:?\*\*\s*\n+---\s*\n+🌸\s*\*\*Ad\*\*\s*🌸[\s\S]*?(?:pollinations\.ai[^\n]*|kofi[^\n]*)\s*(?:\n|$)/i,
    ""
  );

  // 2. Kill any trailing "Support Pollinations" section (with or without the ad emoji).
  out = out.replace(
    /\n+---\s*\n+\*\*Support Pollinations\.AI:?\*\*\*?[\s\S]*$/i,
    ""
  );

  // 3. Kill the standalone 🌸 Ad 🌸 block.
  out = out.replace(
    /\n+🌸\s*\*\*Ad\*\*\s*🌸[\s\S]*?(?:pollinations\.ai|kofi)[^\n]*\s*(?:\n|$)/i,
    ""
  );

  // 4. Kill "Powered by Pollinations.AI" lines.
  out = out.replace(
    /\n+\*\*?Powered by\s+Pollinations\.AI\*?\*?\s*$/i,
    ""
  );

  // 5. Kill any trailing pollinations.ai URL.
  out = out.replace(/\n+https?:\/\/(?:www\.)?pollinations\.ai\/?[^\s]*\s*$/i, "");

  // 6. Kill trailing "— pollinations.ai" attributions.
  out = out.replace(/\s*—\s*pollinations\.ai\s*$/i, "");

  // 7. Kill "image by/generated by/with pollinations" lines.
  out = out.replace(
    /\n+(?:image\s+by|generated\s+(?:by|with))\s+pollinations[^\n]*$/i,
    ""
  );

  // 8. Clean up trailing whitespace / empty markdown separators.
  out = out.replace(/\n+---\s*$/g, "");
  out = out.replace(/\n{3,}$/g, "\n\n");
